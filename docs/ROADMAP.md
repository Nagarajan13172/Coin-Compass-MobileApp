...
| 7.4 | Push notifications | [x] **No push is possible** — no device-token endpoint exists and the backend is not on this machine. Built the deliverable version instead: the app polls `/notifications` on resume and every ~15 min (WorkManager) and raises **local** device notifications, deep-linking via the server's own `link`. First run adopts the feed silently; bursts capped; nothing repeats. Off by default. Permission-denied path device-verified; the granted path needs one human tap. See `PHASE7_4_REPORT.md` |
