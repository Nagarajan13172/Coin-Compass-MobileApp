...
| 7.2 | Google OAuth sign-in | [~] **Blocked on the backend** — the session is an httpOnly cookie, so an external browser keeps it out of Dio's jar and Google refuses embedded WebViews. Needs a one-time-code exchange endpoint (spec in `PHASE7_2_REPORT.md`; no Google Console work required). Shipped: the login screen now reads `GET /auth/providers` instead of hardcoding the button. ⚠️ **Found a live bug — production `redirect_uri` is `http://localhost:4000`, so Google sign-in is broken for web users too.** |
...
| 7.4 | Push notifications | [x] **No push is possible** — no device-token endpoint exists and the backend is not on this machine. Built the deliverable version instead: the app polls `/notifications` on resume and every ~15 min (WorkManager) and raises **local** device notifications, deep-linking via the server's own `link`. First run adopts the feed silently; bursts capped; nothing repeats. Off by default. **Fully device-verified**: adoption is silent, a burst of six rings five, content and money formatting are right, and a tap deep-links. Three device-only defects found and fixed on the way. See `PHASE7_4_REPORT.md` |
| 7.6 | Pay from the app (UPI) | [x] Type an expense, tap **Pay ₹250 with UPI**, choose from an in-app sheet of the payment apps actually installed. Android only — no iOS UPI contract. VPAs remembered on-device (no backend change). Nothing recorded automatically: the deep-link response is not proof of payment. Device-verified up to the write boundary; a real payment is untested. See `PHASE7_6_REPORT.md` |
| 7.7 | Scan a UPI QR | [x] Scan a shop's QR inside the app, confirm who it would pay, fill the expense, then open the payment app with payee **and amount** prefilled. A QR is untrusted: only `pa` is strict, the printed name is a label beside the VPA and never instead of it, and non-INR is refused. Android only. Camera verified on device; an actual scan and the prefilled-amount behaviour of each app remain untested. See `PHASE7_7_REPORT.md` |
| 7.8 | Scan & pay from the nav bar | [x] **Scan** is now a bottom-bar slot (Reports moved to the first More row), and the pre-filled amount is back: the payment app opens on payee + amount + a unique `tr`, and the owner only approves it. A refusal drops one rung — payee-only, then app-only — rather than giving up, and 7.7's blanket conclusion is revisited in the report. The payment then prefills a transaction row the owner saves. **A real payment is still unverified** — that needs a shop and a rupee. See `PHASE7_8_REPORT.md` |
| 7.5 | iOS build | [~] **Builds and signs** (Xcode 26.5, team `A9L6RR9C29`). Was blocked by 7.1: ML Kit needs iOS 15.5 and the project still said 13.0 — bumped, and the Podfile platform pinned. Added the missing `NSFaceIDUsageDescription`, without which iOS terminates the app at the lock screen. **ML Kit ships no arm64 simulator slices, so iOS runs on a real device only.** On-device test still outstanding — no iPhone/iPad attached. See `PHASE7_5_REPORT.md` |
